...
(define (bytes->module bstr)
  ...
  (define functions
    (for/list ([f (in-functions m)]
               #:when (let ([s (value->name/string f)])
      ...
      (let ([counter 0])
        ...
        (for ([bb (in-basic-blocks f)])
          ...
          (for ([insn (in-instructions bb)]
                #:when (and (typeof-ref insn) (unnamed? insn)))
...
(define (make-instruction td insn name type opcode operands)
  ...
  (case opcode
    ...
    [(call)
     (let-values ([(args target) (split-at-right operands 1)])
       ...
       (cond
         ...
         [else
          ...
          (define fname (if (and fname-raw (string-prefix? fname-raw "@")) (substring fname-raw 1) #f))
          (cond
            ; lift ubsan handling
    ...
    [(extractvalue insertvalue)
     (set! operands (append operands (LLVMGetIndices insn)))]
       ...
       (define indices
         (for/list ([idx (cdr operands)])
           (case (LLVMGetTypeKind type)
             [(struct)
              (let* ([elem-idx (bitvector->natural idx)]
             ...
             [else
              (raise 'llvm "unknown GEP type: ~a" (LLVMPrintTypeToString type))])))
       (set! operands (cons (car operands) indices)))]

    ; extract icmp predicate
    [(icmp)
     (set! opcode (list opcode (LLVMGetICmpPredicate insn)))]

    ; add result type to casts
    [(sext zext trunc ptrtoint)
...
(define (lift v td)
  ...
  (cond
    ; strip bitcast
    [(and (equal? kind 'constant-expr)
          (equal? (LLVMGetConstOpcode v) 'bitcast))
     (lift (LLVMGetOperand (value-> v _LLVMConstantExprRef) 0) td)]
    ; lift global initializer
    [(equal? kind 'global-variable)
     (let ([v (value-> v _LLVMGlobalVariableRef)])
       (when (LLVMGetInitializer v)
         (lift (LLVMGetInitializer v) td)))]
    ; lift integer
    [(equal? kind 'constant-int)
     (LLVMConstIntGetSExtValue (value-> v _LLVMConstantIntRef))]
    ; lift string
    [(and (equal? kind 'constant-data-array)
          (LLVMIsConstantString v))
     (LLVMGetAsString v)]
    ; lift struct
    [(equal? kind 'constant-struct)
     (for/list ([op (in-operands v)])
       (lift op td))]
    ; default
    [else
     (value-ref v td)]))

(define builtins (pregexp (string-join (list
...
(define (type->block type td)
  ...
  (case kind
    [(array)
     (marray (LLVMGetArrayLength type) (type->block (LLVMGetElementType type) td))]
    [(struct)
     (mstruct (LLVMABISizeOfType td type)
              (for/list ([i (in-range (LLVMCountStructElementTypes type))])
                (define subtype (LLVMStructGetTypeAtIndex type i))
                (mfield i (LLVMOffsetOfElement td type i) (type->block (LLVMStructGetTypeAtIndex type i) td))))]
    [(integer pointer)
     (mcell (LLVMABISizeOfType td type))]
    [else
     (raise-user-error 'llvm "unknown type for block: ~a" (LLVMPrintTypeToString type))]))
...
(define (split-to-pairs lst)
  (match lst
    [(list x y rest ...)
     (cons (cons x y) (split-to-pairs rest))]
    [_ null]))
  ...
  (case (LLVMGetValueKind v)
    ...
    [(inline-asm)
     ...
     (match (value->string v)
       [(pregexp #px"asm [a-z ]*\"([^\"]*)\", \"([^\"]*)\"$" (list _ template constraint))
...
(define (in-functions m)
  (in-generator
    (let loop ([v (LLVMGetFirstFunction m)])
      (when v
        (yield v)
...
(define (in-globals m)
  (in-generator
    (let loop ([v (LLVMGetFirstGlobal m)])
      (when v
        (yield v)
...
(define (in-arguments f)
  (in-generator
    (let loop ([v (LLVMGetFirstParam f)])
      (when v
        (yield v)
...
(define (in-basic-blocks f)
  (in-generator
    (let loop ([v (LLVMGetFirstBasicBlock f)])
      (when v
        (yield v)
...
(define (in-instructions bb)
  (in-generator
    (let loop ([v (LLVMGetFirstInstruction bb)])
      (when v
        (yield v)
