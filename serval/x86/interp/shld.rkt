...
(define-insn shld-r/m32-r32-imm8 (dst src imm8)
  ...
  (lambda (cpu dst src imm8)
    (interpret-shld cpu dst src (zero-extend imm8 (bitvector 32)))))

; REX.W + 0F A4 /r
(define-insn shld-r/m64-r64-imm8 (dst src imm8)
  #:decode [((rex.w/r r b) (byte #x0F) (byte #xA4) (/r reg r/m) ib)
            (list (gpr64 b r/m) (gpr64 r reg) ib)]
  #:encode (list (rex.w/r src dst) (byte #x0F) (byte #xA4) (/r src dst) imm8)
  (lambda (cpu dst src imm8)
    (interpret-shld cpu dst src (zero-extend imm8 (bitvector 64)))))

; 0F A5 /r
(define-insn shld-r/m32-r32-cl (dst src)
  #:decode [((byte #x0F) (byte #xA5) (/r reg r/m))
            (list (gpr32-no-rex r/m) (gpr32-no-rex reg))]
           [((rex/r r b) (byte #x0F) (byte #xA5) (/r reg r/m))
            (list (gpr32 b r/m) (gpr32 r reg))]
  #:encode (list (rex/r src dst) (byte #x0F) (byte #xA5) (/r src dst))
  (lambda (cpu dst src)
    (interpret-shld cpu dst src (zero-extend (cpu-gpr-ref cpu cl) (bitvector 32)))))
